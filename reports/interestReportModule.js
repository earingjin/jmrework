(function () {
function normalizeNoTargetJobReportData(raw,input={}){const r=raw||{};const p=input.participant||input.personalInfo||{};const cleanArray=value=>Array.isArray(value)?value.filter(item=>item!==null&&item!==undefined):[];const textFrom=value=>typeof value==='string'?value:(value?.question||value?.text||value?.title||'');const scores=noTargetScoresFromInput(input);const top=sortedScoreEntries(input.interestRaw||{});const coreCode=top.slice(0,2).map(([key])=>({'현실형':'R','탐구형':'I','예술형':'A','사회형':'S','진취형':'E','관습형':'C'}[key]||key)).join('');const personality=r.personalityTest||{};const life=r.lifeHistoryTest||{};const normalized={reportTitle:r.reportTitle||'직업선호도검사(L형) 진로 탐색형 분석 리포트',participantInfo:{name:p.name||'',testDate:p.testDate||today(),age:p.age||'',education:p.education||'',currentJob:p.currentJob||'',targetJobStatus:'희망 직무 없음',coreCode:r.participantInfo?.coreCode||coreCode,recommendedJobGroup:r.participantInfo?.recommendedJobGroup||'',strengthSummary:r.participantInfo?.strengthSummary||''},interestTest:{representativeCode:coreCode,shapeAnalysis:r.interestTest?.shapeAnalysis||'',scores:{...scores,...(r.interestTest?.scores||{})},strengths:cleanArray(r.interestTest?.strengths),preferredActivities:cleanArray(r.interestTest?.preferredActivities),avoidActivities:cleanArray(r.interestTest?.avoidActivities),celebrityType:r.interestTest?.celebrityType||''},personalityTest:{...personality,isProvided:personality.isProvided!==undefined?!!personality.isProvided:!!personality.summary,scores:personality.scores||{}},lifeHistoryTest:{...life,isProvided:life.isProvided!==undefined?!!life.isProvided:!!life.summary,scores:life.scores||{}},integratedAnalysis:r.integratedAnalysis||'',strengthExplorationQuestions:cleanArray(r.strengthExplorationQuestions).map(textFrom).filter(Boolean),swot:{strengths:cleanArray(r.swot?.strengths).map(textFrom).filter(Boolean),weaknesses:cleanArray(r.swot?.weaknesses).map(textFrom).filter(Boolean),opportunities:cleanArray(r.swot?.opportunities).map(textFrom).filter(Boolean),threats:cleanArray(r.swot?.threats).map(textFrom).filter(Boolean)},recommendedJobs:cleanArray(r.recommendedJobs),encouragementSlogans:cleanArray(r.encouragementSlogans).map(textFrom).filter(Boolean),aiLifeQuestions:cleanArray(r.aiLifeQuestions),counselorNotice:r.counselorNotice||'본 리포트는 AI가 분석한 데이터이며 내용에 대한 최종 평가는 전문가에게 있습니다.'};normalized.recommendedJobs=normalized.recommendedJobs.map(job=>{const j=job&&typeof job==='object'?job:{title:String(job||'')};return{title:j.title||j.job||'',reason:j.reason||'',relatedStrength:j.relatedStrength||j.strength||'검사 결과 기반 강점과 연결',preparation:j.preparation||j.prep||'관련 채용공고 비교와 필요 역량 확인'}}).filter(job=>job.title);normalized.aiLifeQuestions=normalized.aiLifeQuestions.map(q=>typeof q==='string'?{question:q,intent:'',counselorUse:''}:{question:q?.question||q?.text||'',intent:q?.intent||'',counselorUse:q?.counselorUse||''}).filter(q=>q.question);return normalized}
...
function normalizeTargetInterestReportData(raw,input,target){const r=raw||{};const p=input?.personalInfo||{};const cleanArray=value=>Array.isArray(value)?value.map(item=>String(item||'').trim()).filter(Boolean):[];const normalized={participantInfo:{name:p.name||'',age:p.age||'',education:p.education||'',targetJob:target||r.participantInfo?.targetJob||'',coreCode:r.participantInfo?.coreCode||interestCoreCodeFromData(input),strengthSummary:r.participantInfo?.strengthSummary||''},targetJobCompetencyAnalysis:{fitSummary:String(r.targetJobCompetencyAnalysis?.fitSummary||''),matchingPoints:cleanArray(r.targetJobCompetencyAnalysis?.matchingPoints).slice(0,3),gaps:cleanArray(r.targetJobCompetencyAnalysis?.gaps).slice(0,3)},integratedAnalysis:String(r.integratedAnalysis||r.interestAnalysis||r.lifeCareerAnalysis||''),swot:{strengths:cleanArray(r.swot?.strengths).slice(0,2),weaknesses:cleanArray(r.swot?.weaknesses).slice(0,2),opportunities:cleanArray(r.swot?.opportunities).slice(0,2),threats:cleanArray(r.swot?.threats).slice(0,2)},recommendedJobs:Array.isArray(r.recommendedJobs)?r.recommendedJobs:[],demographicOutlook:String(r.demographicOutlook||''),digitalTransformationOutlook:String(r.digitalTransformationOutlook||''),finalStrategy:String(r.finalStrategy||''),coachingQuestions:cleanArray(r.coachingQuestions).slice(0,10),encouragementSlogans:cleanArray(r.encouragementSlogans).slice(0,4)};normalized.recommendedJobs=normalized.recommendedJobs.slice(0,5).map(job=>({title:job.title||job.job||'',reason:job.reason||'',relatedStrength:job.relatedStrength||job.strength||'',preparation:job.preparation||job.prep||'관련 채용공고를 비교하고 필요한 역량을 확인합니다.'}));return normalized}
...
${renderPdfQuestions(r.aiLifeQuestions, 10)}
<h2>내담자를 위한 응원 메세지</h2>
${safeReportList(r.encouragementSlogans)}</div>`;
}
...
<h2>상담사용 코칭 질문 10가지</h2>
${renderPdfQuestions(r.coachingQuestions, 10)}
<h2>내담자를 위한 응원 메세지</h2>
${safeReportList(r.encouragementSlogans)}</div>`;
}
...
function noTargetGeminiSystemInstruction(){return GEMINI_NO_TARGET_INTEREST_PROMPT.system()}
function noTargetGeminiUserPrompt(input){return GEMINI_NO_TARGET_INTEREST_PROMPT.user(input)}
...
function interestJsonSchemaForRepair(kind){const common={participantInfo:'object',integratedAnalysis:'string',swot:{strengths:'string[]',weaknesses:'string[]',opportunities:'string[]',threats:'string[]'},recommendedJobs:[{title:'string',reason:'string',relatedStrength:'string',preparation:'string'}],encouragementSlogans:'string[]'};return kind==='target'?{...common,targetJobCompetencyAnalysis:{fitSummary:'string',matchingPoints:'string[]',gaps:'string[]'},demographicOutlook:'string',digitalTransformationOutlook:'string',finalStrategy:'string',coachingQuestions:'string[]'}:{...common,interestSummary:{shapeAnalysis:'string',counselorReferenceType:'string'},personalitySummary:'string',lifeHistorySummary:'string',strengthExplorationQuestions:'string[]',aiLifeQuestions:[{question:'string',intent:'string',counselorUse:'string'}]}}
function recordInterestJsonRecoveryEvent(payload={}){const recoveryType=payload.regenerateAttempted?REPORT_RECOVERY_TYPE.FULL_REGENERATION:payload.jsonRepairAttempted?REPORT_RECOVERY_TYPE.AI_JSON_REPAIR:payload.autoRepairAttempted?REPORT_RECOVERY_TYPE.CODE_REPAIR:REPORT_RECOVERY_TYPE.NONE;const errorType=payload.parseSuccess===false?(payload.jsonRepairAttempted?REPORT_ERROR_TYPE.JSON_REPAIR_FAILED:REPORT_ERROR_TYPE.JSON_PARSE_ERROR):(recoveryType!==REPORT_RECOVERY_TYPE.NONE?REPORT_ERROR_TYPE.JSON_PARSE_ERROR:undefined);noteReportGenerationIssue({errorType,modelName:payload.modelName,retryCount:state.reportGenerationRetryCount,retryReason:errorType,recoveryType})}
function isInterestJsonParseError(err){return['JSON_PARSE_ERROR','JSON_REPAIR_FAILED','TRUNCATED_JSON'].includes(err?.errorType)||err instanceof SyntaxError||String(err?.message||'').includes('JSON')}
async function requestInterestJsonRepair(rawText,schema,modelName){const body={contents:[{role:'user',parts:[{text:`내용을 새로 만들지 말고 JSON 문법만 고쳐라. JSON 객체만 출력하라.\n\n필요한 schema:\n${JSON.stringify(schema)}\n\n깨진 JSON 원문:\n${String(rawText||'')}`}]}],generationConfig:{temperature:0,topP:.1,responseMimeType:'application/json'}};const res=await window.AI_GATEWAY.generateContent({model:modelName,body,request:fetchGeminiWithRetry});const data=await res.json().catch(()=>({}));if(!res.ok){const err=new Error(data?.error?.message||`Gemini JSON repair API 오류 (${res.status})`);err.status=res.status;throw err}const candidate=data?.candidates?.[0]||{};assertGeminiCandidateCompleted(candidate,'JSON repair');const text=(candidate?.content?.parts||[]).map(part=>part.text||'').join('\n').trim();if(!text)throw new Error('JSON repair 응답이 비어 있습니다.');return text}
async function parseInterestGeminiJsonWithRecovery(text,{schema,modelName,allowJsonRepair=true}={}){const startedAt=Date.now();try{const result=parseGeminiJsonDetailed(text);recordInterestJsonRecoveryEvent({modelName,parseSuccess:true,autoRepairAttempted:result.autoRepairAttempted,autoRepairSuccess:result.autoRepairSuccess,jsonRepairAttempted:false,jsonRepairSuccess:false,regenerateAttempted:false,regenerateSuccess:false,durationMs:Date.now()-startedAt});return result.value}catch(autoErr){logInterestJsonParseFailure(autoErr,text);recordInterestJsonRecoveryEvent({modelName,parseSuccess:false,autoRepairAttempted:true,autoRepairSuccess:false,jsonRepairAttempted:false,jsonRepairSuccess:false,regenerateAttempted:false,regenerateSuccess:false,errorMessage:autoErr?.message||String(autoErr),durationMs:Date.now()-startedAt});if(!allowJsonRepair)throw autoErr;try{const repairedText=await requestInterestJsonRepair(text,schema,modelName);const repaired=parseGeminiJsonDetailed(repairedText);recordInterestJsonRecoveryEvent({modelName,parseSuccess:true,autoRepairAttempted:repaired.autoRepairAttempted,autoRepairSuccess:repaired.autoRepairSuccess,jsonRepairAttempted:true,jsonRepairSuccess:true,regenerateAttempted:false,regenerateSuccess:false,durationMs:Date.now()-startedAt});return repaired.value}catch(repairErr){recordInterestJsonRecoveryEvent({modelName,parseSuccess:false,autoRepairAttempted:true,autoRepairSuccess:false,jsonRepairAttempted:true,jsonRepairSuccess:false,regenerateAttempted:false,regenerateSuccess:false,errorMessage:repairErr?.message||String(repairErr),durationMs:Date.now()-startedAt});repairErr.errorType='JSON_REPAIR_FAILED';repairErr.cause=repairErr.cause||autoErr;throw repairErr}}}
async function requestGeminiNoTargetJobReportData(input,options={}){const modelName=getGeminiModel('interest');const body={system_instruction:{parts:[{text:noTargetGeminiSystemInstruction()}]},contents:[{role:'user',parts:[{text:noTargetGeminiUserPrompt(input)}]}],generationConfig:{temperature:.12,topP:.65,responseMimeType:'application/json'}};const res=await fetchGeminiInterestWithModelFallback(body);const data=await res.json().catch(()=>({}));if(!res.ok)throw new Error(data?.error?.message||`Gemini API 오류 (${res.status})`);const candidate=data?.candidates?.[0]||{};assertGeminiCandidateCompleted(candidate,'직업선호도검사 희망직무 없음');const text=(candidate?.content?.parts||[]).map(part=>part.text||'').join('\n').trim();if(!text)throw new Error('Gemini 응답에서 JSON 내용을 찾지 못했습니다.');const parsed=await parseInterestGeminiJsonWithRecovery(text,{schema:interestJsonSchemaForRepair('noTarget'),modelName,allowJsonRepair:options.allowJsonRepair!==false});return{...normalizeNoTargetJobReportDataAligned(parsed,input),tokenUsage:normalizeTokenUsage(data.usageMetadata)}}
async function generateNoTargetJobInterestReport(p){
  ...
  try{
    ...
    let reportData;
    try{
      reportData=await requestGeminiNoTargetJobReportData(noTargetInput);
    }catch(err){
      if(!isInterestJsonParseError(err))throw err;
      const startedAt=Date.now();
      recordInterestJsonRecoveryEvent({modelName:getGeminiModel('interest'),parseSuccess:false,autoRepairAttempted:true,autoRepairSuccess:false,jsonRepairAttempted:true,jsonRepairSuccess:false,regenerateAttempted:true,regenerateSuccess:false,errorMessage:err?.message||String(err),durationMs:0});
      state.reportGenerationRetryCount++;
      try{
        reportData=await requestGeminiNoTargetJobReportData(noTargetInput,{allowJsonRepair:false});
        recordInterestJsonRecoveryEvent({modelName:getGeminiModel('interest'),parseSuccess:true,autoRepairAttempted:true,autoRepairSuccess:true,jsonRepairAttempted:true,jsonRepairSuccess:false,regenerateAttempted:true,regenerateSuccess:true,durationMs:Date.now()-startedAt});
      }catch(regenerateErr){
        recordInterestJsonRecoveryEvent({modelName:getGeminiModel('interest'),parseSuccess:false,autoRepairAttempted:true,autoRepairSuccess:false,jsonRepairAttempted:true,jsonRepairSuccess:false,regenerateAttempted:true,regenerateSuccess:false,errorMessage:regenerateErr?.message||String(regenerateErr),durationMs:Date.now()-startedAt});
        throw regenerateErr;
      }
    }
...
function getInterestClient(){const hasTarget=val('interestHasTarget')==='yes';const target=hasTarget?val('interestTarget'):'검사 결과 기반 추천 직무';return{id:'session_interest',name:val('interestPersonName')||'내담자',age:val('interestPersonAge')||'미입력',status:'검사결과 분석',docStatus:'직접 입력',target,hasTarget,career:'상담사 직접 입력 검사 결과 기반',memo:val('interestMemo'),createdAt:today()}}function toggleInterestTargetInput(){const wrap=document.getElementById('interestTargetWrap');const input=document.getElementById('interestTarget');const hasTarget=val('interestHasTarget')==='yes';if(wrap)wrap.style.display=hasTarget?'block':'none';if(input&&!hasTarget)input.value=''}
...
async function requestGeminiTargetInterestReportData(input,target,options={}){const modelName=getGeminiModel('interest');const body={system_instruction:{parts:[{text:GEMINI_TARGET_INTEREST_PROMPT.system()}]},contents:[{role:'user',parts:[{text:GEMINI_TARGET_INTEREST_PROMPT.user(input)}]}],generationConfig:{temperature:.12,topP:.65,responseMimeType:'application/json'}};const res=await fetchGeminiInterestWithModelFallback(body);const data=await res.json().catch(()=>({}));if(!res.ok)throw new Error(data?.error?.message||`Gemini API 오류 (${res.status})`);const candidate=data?.candidates?.[0]||{};assertGeminiCandidateCompleted(candidate,'직업선호도검사 희망직무 있음');const text=(candidate?.content?.parts||[]).map(part=>part.text||'').join('\n').trim();if(!text)throw new Error('Gemini 응답에서 JSON 내용을 찾지 못했습니다.');const parsed=await parseInterestGeminiJsonWithRecovery(text,{schema:interestJsonSchemaForRepair('target'),modelName,allowJsonRepair:options.allowJsonRepair!==false});return{...normalizeTargetInterestReportData(parsed,input,target),tokenUsage:normalizeTokenUsage(data.usageMetadata)}}
async function generateGeminiInterestReport(p,target,hasTarget){if(!hasTarget)return generateNoTargetJobInterestReport(p);const manualData=collectInterestManualData(true,target);const btn=document.getElementById('interestButton');const oldText=btn?btn.textContent:'';if(btn){btn.disabled=true;btn.textContent='리포트 생성 중...'}try{let reportData;try{reportData=await requestGeminiTargetInterestReportData(manualData,target)}catch(err){if(!isInterestJsonParseError(err))throw err;const startedAt=Date.now();recordInterestJsonRecoveryEvent({modelName:getGeminiModel('interest'),parseSuccess:false,autoRepairAttempted:true,autoRepairSuccess:false,jsonRepairAttempted:true,jsonRepairSuccess:false,regenerateAttempted:true,regenerateSuccess:false,errorMessage:err?.message||String(err),durationMs:0});state.reportGenerationRetryCount++;try{reportData=await requestGeminiTargetInterestReportData(manualData,target,{allowJsonRepair:false});recordInterestJsonRecoveryEvent({modelName:getGeminiModel('interest'),parseSuccess:true,autoRepairAttempted:true,autoRepairSuccess:true,jsonRepairAttempted:true,jsonRepairSuccess:false,regenerateAttempted:true,regenerateSuccess:true,durationMs:Date.now()-startedAt})}catch(regenerateErr){recordInterestJsonRecoveryEvent({modelName:getGeminiModel('interest'),parseSuccess:false,autoRepairAttempted:true,autoRepairSuccess:false,jsonRepairAttempted:true,jsonRepairSuccess:false,regenerateAttempted:true,regenerateSuccess:false,errorMessage:regenerateErr?.message||String(regenerateErr),durationMs:Date.now()-startedAt});throw regenerateErr}}return{title:`${reportData.participantInfo.name||manualData.personalInfo.name} 직업선호도검사 리포트 (${target})`,html:renderTargetInterestReportFromData(reportData),participantName:reportData.participantInfo.name||manualData.personalInfo.name,tokenUsage:reportData.tokenUsage}}finally{if(btn){btn.disabled=false;btn.textContent=oldText||'직업선호도검사 리포트 생성'}}}
  ...
  window.REPORT_MODULES.register('interest', {
    ...
    async generate() {
      ...
      try {
        ...
        const title = hasTargetJob ? `${p.name} 직업선호도검사 리포트 (${target})` : generated.title;
