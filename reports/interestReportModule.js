...
function recordInterestJsonRecoveryEvent(payload={}){recordUsageEvent('json_parse_recovery',{reportType:'interest',errorType:'JSON_PARSE_ERROR',retryCount:state.reportGenerationRetryCount,...payload})}
function isInterestJsonParseError(err){return err?.errorType==='JSON_PARSE_ERROR'||err?.errorType==='TRUNCATED_JSON'||err instanceof SyntaxError||String(err?.message||'').includes('JSON')}
