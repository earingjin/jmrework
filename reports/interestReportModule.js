...
function renderJobExampleInfoHint() {
  const message = '더 자세한 결과를 원하는 경우, 왼쪽 하단의 <참여자 이슈>에 더 많은 정보를 입력해 주세요.';
...
function interestManualForm(p){
  ...
  const memoField=`<div class="field"><label>참여자 이슈*</label><div class="small">1. 참여자의 관심 분야, 이전 경력, 희망 근무 조건<br>2. 추천하지 않는 직업군 <br>(전공과 다른 분야를 희망하는 경우 등)<br>3. 자세하게 작성할수록 더 세분화된 직업 추천 가능</div><textarea id="interestMemo" placeholder=""></textarea></div>`;
