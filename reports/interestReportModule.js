...
async function requestGeminiNoTargetJobReportData(apiKey,input){if(!apiKey)throw new Error('Gemini API 키를 입력하거나 GEMINI_API_KEY 상수를 설정해주세요.');const body={system_instruction:{parts:[{text:noTargetGeminiSystemInstruction()}]},contents:[{role:'user',parts:[{text:noTargetGeminiUserPrompt(input)}]}],generationConfig:{temperature:.12,topP:.65,responseMimeType:'application/json'}};const res=await fetchGeminiInterestWithModelFallback(apiKey,body);const data=await res.json().catch(()=>({}));if(!res.ok)throw new Error(data?.error?.message||`Gemini API 오류 (${res.status})`);const text=(data?.candidates?.[0]?.content?.parts||[]).map(part=>part.text||'').join('\n').trim();if(!text)throw new Error('Gemini 응답에서 JSON 내용을 찾지 못했습니다.');return normalizeNoTargetJobReportDataAligned(parseGeminiJson(text),input)}
async function generateNoTargetJobInterestReport(p){
  ...
  try{
    ...
    const apiKey=val('interestApiKey')||localStorage.getItem('GEMINI_API_KEY')||GEMINI_API_KEY;
    if(val('interestApiKey'))localStorage.setItem('GEMINI_API_KEY',val('interestApiKey'));
    const reportData=await requestGeminiNoTargetJobReportData(apiKey,noTargetInput);
    const participantName=reportData.participantInfo?.name||p?.name||'내담자';
    return{title:`${participantName} 직업선호도검사 리포트 (희망 직무 없음)`,html:renderNoTargetJobReportSafe(reportData),participantName};
...
async function requestGeminiTargetInterestReportData(apiKey,input,target){if(!apiKey)throw new Error('Gemini API 키를 입력해주세요.');const body={system_instruction:{parts:[{text:GEMINI_TARGET_INTEREST_PROMPT.system()}]},contents:[{role:'user',parts:[{text:GEMINI_TARGET_INTEREST_PROMPT.user(input)}]}],generationConfig:{temperature:.12,topP:.65,responseMimeType:'application/json'}};const res=await fetchGeminiInterestWithModelFallback(apiKey,body);const data=await res.json().catch(()=>({}));if(!res.ok)throw new Error(data?.error?.message||`Gemini API 오류 (${res.status})`);const candidate=data?.candidates?.[0]||{};if(candidate.finishReason==='MAX_TOKENS')throw new Error('Gemini 응답이 길이 제한으로 잘려 JSON을 완성하지 못했습니다. 입력 메모를 줄이거나 다시 생성해 주세요.');const text=(candidate?.content?.parts||[]).map(part=>part.text||'').join('\n').trim();if(!text)throw new Error('Gemini 응답에서 JSON 내용을 찾지 못했습니다.');return normalizeTargetInterestReportData(parseGeminiJson(text),input,target)}
async function generateGeminiInterestReport(p,target,hasTarget){if(!hasTarget)return generateNoTargetJobInterestReport(p);const apiKey=val('interestApiKey');const manualData=collectInterestManualData(true,target);if(apiKey)localStorage.setItem('GEMINI_API_KEY',apiKey);const btn=document.getElementById('interestButton');const oldText=btn?btn.textContent:'';if(btn){btn.disabled=true;btn.textContent='리포트 생성 중...'}try{const reportData=await requestGeminiTargetInterestReportData(apiKey,manualData,target);return{title:`${reportData.participantInfo.name||manualData.personalInfo.name} 직업선호도검사 리포트 (${target})`,html:renderTargetInterestReportFromData(reportData),participantName:reportData.participantInfo.name||manualData.personalInfo.name}}finally{if(btn){btn.disabled=false;btn.textContent=oldText||'직업선호도검사 리포트 생성'}}}
