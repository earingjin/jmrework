...
async function updateAccountPassword(id, newPassword) {
  ...
  const res = await pool.query('UPDATE accounts SET password_hash = $1 WHERE id = $2 RETURNING id', [hash, id]);
  return res.rowCount === 1;
}

module.exports = { init, query, insertUsageEvents, findAccountByLogin, findAccountById, listAccounts, replaceCounselorAccounts, createAccount, updateAccount, deleteAccount, updateAccountPassword, enabled };
