...
function subNavButton(definition) {
  ...
  return `<button class="${state.active === 'modules' && state.activeModule === definition.id ? 'active' : ''}" onclick="setReportModule('${definition.id}')"><span>${definition.available ? label : label + ' (점검중)'}</span></button>`;
}
