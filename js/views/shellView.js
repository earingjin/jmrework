...
function shellTemplate() {
  ...
  return `<div class="shell"><aside class="sidebar no-print">${sideBrand}<nav class="nav"><button class="${state.active === 'modules' ? 'active' : ''}" onclick="toggleReportMenu()"><span>리포트 생성</span><span>${state.reportMenuOpen ? '⌃' : '⌄'}</span></button><div class="subnav" ${reportMenuStyle}>${reportSubNavHtml()}</div>${navButton('account', '내 계정')}</nav><div style="margin-top:20px;padding:10px"><button class="btn secondary full" onclick="logout()">로그아웃</button></div><div class="sidebar-copyright">© 2026 JMCareer. All Rights Reserved.</div></aside><main class="main">${activeMainSectionHtml()}</main></div>`;
}
