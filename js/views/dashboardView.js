...
function dashboardSection() {
  ...
  return `<section id="section-dashboard" class="section"><div class="ai-workspace"><div class="workspace-hero"><div><div class="workspace-kicker">RE:WORK AI COUNSELING</div><h1>안녕하세요, ${escapeHtml(name)}님.<br><span class="hero-emphasis">오늘도 AI와 함께 상담 리포트를 빠르게 생성해보세요.</span></h1><p>AI가 초안을 만들고, 상담사가 바로 편집 가능한 리포트를 생성합니다.</p></div><div class="workspace-visual"><div class="workspace-logo-card"><img src="${BRAND_LOGO}" alt="${BRAND_NAME}" class="workspace-logo"><div class="workspace-report-lines"><span></span><span></span><span></span></div></div></div></div><div class="workspace-block"><div class="workspace-head"><div><h3>상담 워크플로우</h3><p>내담자 정보 확인부터 리포트 전달까지 한 화면에서 빠르게 이어갑니다.</p></div></div><div class="workflow-grid">${workflowStep('intake', '정보 입력', '검사 결과 및 상담 메모를 입력합니다.')}${workflowStep('analyze', 'AI 분석', '검사 결과와 경력 조건을 통합합니다.')}${workflowStep('draft', '초안 편집', 'AI 리포트 초안을 상담사가 다듬습니다.')}${workflowStep('deliver', '상담 진행', '더욱 체계적인 상담 진행을 돕습니다.')}</div></div><div class="workspace-block"><div class="workspace-head"><div><h3>빠른 실행</h3><p>자주 사용하는 AI 리포트 생성 화면으로 바로 이동합니다.</p></div></div><div class="quick-grid">${dashboardQuickActionsHtml()}</div></div><div class="workspace-block"><div class="workspace-head"><div><h3>AI 지원 기능</h3><p>생성부터 편집, 저장까지 상담사 업무 흐름에 맞춰 지원합니다.</p></div></div><div class="ai-support-tags"><span class="ai-support-tag">검사 결과 통합 분석</span><span class="ai-support-tag">AI 초안 자동 생성</span><span class="ai-support-tag">상담사 편집 가능</span><span class="ai-support-tag">PDF 저장</span></div></div></div></section>`;
}
