...
- **, -, #, > 같은 마크다운 강조 부호나 AI가 작성한 티가 나는 장식 기호를 문장 앞뒤에 붙이지 마세요.
...
- 성격검사 및 생활사 검사가 입력된 경우 recommendedJobs.relatedStrength 중 최소 2개에는 성격검사와 생활사 검사에서 드러난 일하는 방식 또는 보완점을 직업흥미 결과와 함께 연결하세요.
- recommendedJobs.preparation은 준비 과제 및 필요한 경우 구체적인 자격증 추천까지 1문장으로 작성하세요.
- aiLifeQuestions는 정확히 15개의 문자열 배열로 작성하세요. 객체가 아니라 문자열만 넣으세요.
- aiLifeQuestions 1~10번은 "경험·강점 연결 질문"으로 작성하되, 내담자의 실제 경험을 구체적으로 꺼내고 그 경험이 어떤 강점으로 반복되는지 연결하도록 돕는 질문이어야 합니다.
- aiLifeQuestions 1~10번은 질문 앞부분에 직업흥미코드의 유형과 특성을 한 줄로 짧게 설명하고, 그 특성에 맞춰 경험을 묻는 방식으로 작성하세요.
- aiLifeQuestions 1~10번은 직업흥미검사 결과와 상담사 추가 메모에 적힌 시사점을 함께 참조하여, 내담자에게 실제로 해당될 가능성이 높은 경험을 묻는 질문으로 작성하세요.
- aiLifeQuestions 1~10번에는 "시간 가는 줄 모르고 몰입했던 경험", "결과와 상관없이 뿌듯했던 경험", "큰 노력을 들이지 않았는데 좋은 결과가 나온 경험", "나를 잘 아는 사람들이 내 특성을 부르는 애칭이나 별명"을 확인하는 질문을 포함하세요.
- aiLifeQuestions 1~10번은 특정 직업을 바로 정답처럼 제시하지 말고, 경험 속에서 반복되는 흥미, 강점, 일하는 방식, 주변의 인정 단서를 찾아 내담자가 자신의 강점을 말로 정리할 수 있게 묻는 방식으로 작성하세요.
- aiLifeQuestions 11~15번은 "현실적 진로 대안 연결 질문"으로 작성하세요. 1~10번의 경험·강점 연결 답변과 검사 해석을 바탕으로, 내담자가 앞으로의 진로 방향과 다음 회기 목표를 설정하도록 돕는 질문이어야 합니다.
- aiLifeQuestions 11~15번은 내담자가 선택 가능한 현실적 진로 대안, 우선 확인할 직무군, 탐색 순서, 준비 과제, 다음 회기까지 해볼 행동을 구체화하도록 작성하세요.
- aiLifeQuestions 11~15번에는 관심 업종, 관심 뉴스, 관련 카페나 커뮤니티를 조사해오는 방법을 구체화하는 질문을 포함하세요.
- aiLifeQuestions 11~15번에는 정부지원사업, 정부지원 직업교육, 국비교육, 자격증 등 활용 가능한 외부 자원을 찾아보도록 돕는 질문을 포함하세요.
- aiLifeQuestions 11~15번은 직업흥미검사의 낮은 점수도 가볍게 짚어, 내담자가 피하고 싶은 업무환경이나 기피 조건을 확인하는 질문으로 활용해도 좋습니다.
