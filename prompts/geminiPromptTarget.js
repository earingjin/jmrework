...
- demographicOutlook
- digitalTransformationOutlook
- finalStrategy.jobInfoExploration
- finalStrategy.competencyPreparation
- finalStrategy.applicationReview
- coachingQuestions
...
- targetJobCompetencyAnalysis.fitSummary, targetJobCompetencyAnalysis.matchingPoints, targetJobCompetencyAnalysis.gaps는 각각 화면에서 2~3문장 분량을 넘지 않도록 간결하게 작성하세요.
- targetJobCompetencyAnalysis.matchingPoints와 targetJobCompetencyAnalysis.gaps는 3개 항목 전체를 합쳤을 때 2~3문장처럼 읽히도록 각 항목을 짧게 작성하세요.
- targetJobCompetencyAnalysis.matchingPoints와 targetJobCompetencyAnalysis.gaps의 각 항목은 개조식, 키워드 나열, 짧은 메모가 아니라 자연스러운 서술식 완성 문장으로 작성하세요.
- targetJobCompetencyAnalysis.matchingPoints와 targetJobCompetencyAnalysis.gaps에는 번호, 불릿 기호, 콜론으로 시작하는 소제목을 넣지 말고 상담사가 바로 읽을 수 있는 문장형 설명만 넣으세요.
- SWOT는 strengths, weaknesses, opportunities, threats 각각 정확히 2개씩 작성하세요. 각 항목은 1문장으로 제한하세요.
- recommendedJobs는 정확히 5개 작성하세요. 희망직무, 직업흥미 점수, 입력된 성격검사와 생활사, 나이, 학력, 전공, 자격증을 함께 고려하세요.
...
- digitalTransformationOutlook는 2~3문장씩 2문단으로 작성하고, 일반론이 아니라 입력된 검사 결과와 희망직무를 연결하세요.
- finalStrategy는 독립된 큰 항목인 "3단계 실행 전략"에 들어갈 내용입니다. 단계 제목은 코드에서 고정 렌더링하므로 AI는 제목을 만들지 말고 각 단계의 실행 문장만 채우세요.
- finalStrategy.jobInfoExploration은 "1단계: 직무 정보 탐색"에 들어갈 실행 문장 2개입니다. 채용공고 비교, 직무 정보 확인, 실제 업무환경 탐색처럼 내담자가 바로 할 수 있는 행동으로 작성하세요.
- finalStrategy.competencyPreparation은 "2단계: 역량 준비"에 들어갈 실행 문장 2개입니다. 부족 역량 선정, 자격요건 확인, 관련 교육 검색, 경험 사례 정리처럼 준비 행동으로 작성하세요.
- finalStrategy.applicationReview은 "3단계: 지원 실행 및 점검"에 들어갈 실행 문장 2개입니다. 지원 실행, 결과 기록, 다음 상담 전 점검, 보완 과제 확인처럼 후속 행동으로 작성하세요.
- finalStrategy의 각 배열에는 문자열 2개만 넣으세요. 번호, 불릿 기호, 단계 제목, 소제목, 설명문은 넣지 마세요.
